...
import com.shazcom.gps.app.databinding.FragmentAccountBinding
...
class Account : BaseFragment(), KodeinAware {
    ...
    private fun showData(data: UserDataResponse?) {
        binding. accountEmail.text = String.format(getString(R.string.email_n_a), "${data?.email}")
        binding.  plan.text = String.format(getString(R.string.plan_n_a), "${data?.plan}")

        if (data?.devices_limit != null) {
            if (data?.devices_limit == 0) {
                binding. device.text = String.format(getString(R.string.devices_limit_n_a), "Unlimited")
            } else {
                binding. device.text =
